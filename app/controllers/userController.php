...
// Verificar si el cargo es Vendedor, entonces se hace obligatorio el campo "usuario_caja"
        if ($cargo === 'Vendedor') {
            $caja = $this->limpiarCadena($_POST['usuario_caja']);
            if ($caja == "") {
                $alerta = [
                    "tipo" => "simple",
                    "titulo" => "Ocurrió un error inesperado",
                    "texto" => "La caja de ventas es obligatoria para el cargo de Vendedor",
                    "icono" => "error"
                ];
                return json_encode($alerta);
                exit();
            ...
            // Si el cargo no es "Vendedor", no se requiere caja
            $caja = null; // o lo puedes dejar vacío si es preferido
        }
